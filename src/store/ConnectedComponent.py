...
class ConnectedComponent:
    """
    V.1.0.2. Component that is connected to the pydux store.
    ...
    def _updater(self):
        ...
        if hasattr(self, '_get_own_state') and hasattr(self, '_on_state_update'):
            ...
            # --------------------------Lists, Dicts, OrderedDicts---------------
            else:
                if new_state is not self._cached:
                    # print(f'updater {self.__class__} and self._cached is not new_state')
                    # --------------------------Ordered Dict---------------
                    if isinstance(new_state, OrderedDict):
                    ...
                    # --------------------------List---------------
                        ...
                        if diff < 0:  # если стало меньше записей
                            ...
                            for ind, new_state_item in enumerate(new_state):
                                if isinstance(new_state[ind], (int, float, str, bool)):
                                    if self._cached[ind] != new_state[ind]:
                                        list_of_indexes_of_updated_items.append(ind)
                                else:
                                    if self._cached[ind] is not new_state[ind]:
                                        list_of_indexes_of_updated_items.append(ind)
