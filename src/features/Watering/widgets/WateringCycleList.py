...
def create_watering_cycle(ID, data, index, on_update, on_delete, parent):
    return WateringCycle(index=index + 1,
                         data=data,
...
class WateringCycleList(ConnectedToStoreComponent, QWidget):
    ...
    def _on_delete_item(self, ID):
        self._dispatch({'type': 'wateringcycles/DELETE_ITEM', 'payload': ID})

    def _on_update_item(self, ID, new_data):
        self._dispatch({'type': 'wateringcycles/UPDATE_ITEM', 'payload': {'ID': ID, 'new_data': new_data}})
